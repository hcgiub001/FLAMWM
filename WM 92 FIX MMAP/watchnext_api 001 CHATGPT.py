...
# ═══════════════════════════════════════════════════════════════════════════
# Binary mapping  (sorted u32 key→value files, O(log n) lookup via bisect)
# ═══════════════════════════════════════════════════════════════════════════
...
class PackedMapping:
    ...
    RAM mode  — uses a 65 K-bucket direct-map index for near-O(1) lookup.
    mmap mode — uses a sample array + manual binary search (no bisect on
                memoryview, which faults on some Android/Kodi CPython builds).
    """
    __slots__ = (
    ...
    _DIRECT_BITS = 16                       # 65 536 buckets
    _DIRECT_SIZE = 1 << _DIRECT_BITS
    ...
    def load_ram(self, spec: Dict[str, str]) -> None:
        """Load all files fully into RAM arrays with a direct-map index."""
        self.close()
        self._is_mmap = False
    ...
    def load_mmap(self, spec: Dict[str, str]) -> None:
        ...
        for attr, key in (("_keys", "keys"), ("_vals", "values")):
            ...
            count = size // 4                       # aligned element count
            fo = open(path, "rb")
            self._fileobjs.append(fo)
            mm = mmap.mmap(fo.fileno(), 0, access=mmap.ACCESS_READ)
            self._mmaps.append(mm)
            # Slice to count*4 bytes so .cast("I") is always aligned
            setattr(self, attr, memoryview(mm)[:count * 4].cast("I"))
    ...
    def lookup(self, key: int) -> Optional[int]:
        ...
        if self._is_mmap:
            return self._lookup_mmap(key)
        return self._lookup_ram(key)

    # ── RAM path: direct-map bucket → bisect within small range ──

    def _build_direct_map(self) -> None:
        """Build a 65 K-bucket index: bucket[b] = first position where
        (key >> 16) >= b.  Gives O(1) range narrowing before bisect."""
        keys = self._keys
        n = self._count
        bits = self._DIRECT_BITS
        dmap = array("I", [0]) * (self._DIRECT_SIZE + 1)
        prev_bucket = 0
        for i in range(n):
            b = keys[i] >> bits
            while prev_bucket <= b:
                dmap[prev_bucket] = i
                prev_bucket += 1
        while prev_bucket <= self._DIRECT_SIZE:
            dmap[prev_bucket] = n
            prev_bucket += 1
        self._direct_map = dmap

    def _lookup_ram(self, key: int) -> Optional[int]:
        dmap = self._direct_map
        keys = self._keys
        if dmap is not None:
            bucket = key >> self._DIRECT_BITS
            lo = dmap[bucket]
            hi = dmap[bucket + 1]
        else:
            lo, hi = 0, self._count
    ...
    # ── mmap path: sample → manual binary search (no bisect on memoryview) ──

    def _lookup_mmap(self, key: int) -> Optional[int]:
        keys = self._keys
        count = self._count
        sample = self._sample

        # Narrow range via sample array (manual upper-bound)
        ...
        # Manual binary search within [lo, hi) — no bisect on memoryview
        while lo < hi:
            mid = (lo + hi) >> 1
            v = keys[mid]
            if v < key:
