...
class PackedMapping:
    ...
    A sample array (every Nth key) narrows the bisect range so lookups
    touch only a small slice — fast in both RAM and mmap modes.

    RAM mode:  keys & values are array('I') in memory.
    mmap mode: keys & values are mmap-backed memoryview objects.

    Both modes use the same sample-based binary search path.
    """
    __slots__ = (
    ...
    def load_ram(self, spec: Dict[str, str]) -> None:
        """Load all files fully into RAM arrays."""
        self.close()
        self._is_mmap = False
    ...
    def lookup(self, key: int) -> Optional[int]:
        ...
        # Narrow the search range using the sample array.
        sample = self._sample
