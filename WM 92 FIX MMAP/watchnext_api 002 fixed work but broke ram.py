...
# Direct-map bucket size for RAM mode (2^16 = 65536 buckets)
_DIRECT_MAP_BITS = 16
_DIRECT_MAP_SIZE = 1 << _DIRECT_MAP_BITS
_DIRECT_MAP_MASK = _DIRECT_MAP_SIZE - 1
...
# ═══════════════════════════════════════════════════════════════════════════
# Manual binary search (safe on memoryview — no bisect/PySequence_GetItem)
# ═══════════════════════════════════════════════════════════════════════════
def _binary_search_block(keys, key: int, lo: int, hi: int) -> int:
    """
...
def _upper_bound_sample(sample, key: int) -> int:
    """
...
# ═══════════════════════════════════════════════════════════════════════════
# Direct-map (hash-bucket index) for RAM mode — O(~16) lookups
# ═══════════════════════════════════════════════════════════════════════════
def _build_direct_map(keys: array) -> List[Tuple[int, int]]:
    """
    Build a 65536-bucket index over `keys`.
    Each bucket stores (lo, hi) — the half-open range of indices whose
    key hashes into that bucket.  Because the keys are sorted, all entries
    in one bucket are contiguous.
    """
    n = len(keys)
    table: List[Tuple[int, int]] = [(0, 0)] * _DIRECT_MAP_SIZE
    if n == 0:
        return table

    # Walk the sorted keys and record where each bucket starts/ends.
    prev_bucket = keys[0] & _DIRECT_MAP_MASK
    run_start = 0
    for i in range(1, n):
        bucket = keys[i] & _DIRECT_MAP_MASK
        if bucket != prev_bucket:
            table[prev_bucket] = (run_start, i)
            # Fill any empty buckets in-between with empty ranges.
            for b in range(prev_bucket + 1, bucket):
                table[b] = (i, i)
            prev_bucket = bucket
            run_start = i
    table[prev_bucket] = (run_start, n)
    # Fill trailing empty buckets.
    for b in range(prev_bucket + 1, _DIRECT_MAP_SIZE):
        table[b] = (n, n)
...
class PackedMapping:
    ...
    RAM mode:  keys & values are array('I') in memory.
               A 65 K direct-map bucket index gives O(log 16) lookups.
    mmap mode: keys & values are mmap-backed memoryview objects.
               A sample array (every Nth key) narrows the search range,
               and a manual binary search avoids bisect/memoryview issues.
    """
    __slots__ = (
    ...
    def __init__(self) -> None:
        ...
        self._direct_map: Optional[List[Tuple[int, int]]] = None
    ...
    def load_ram(self, spec: Dict[str, str]) -> None:
        """Load all files fully into RAM arrays + build direct-map index."""
        self.close()
        self._is_mmap = False
    ...
    def lookup(self, key: int) -> Optional[int]:
        """Find the value for `key`, or None.  O(log n) via binary search."""
        keys = self._keys
        if keys is None:
            return None
        ...
        # --- RAM mode: use direct-map for O(log ~16) lookup ---
        dmap = self._direct_map
        if dmap is not None:
            bucket = key & _DIRECT_MAP_MASK
            lo, hi = dmap[bucket]
            if lo >= hi:
                return None
            i = _binary_search_block(keys, key, lo, hi)
            return self._vals[i] if i >= 0 else None

        # --- mmap mode: narrow via sample, then manual binary search ---
        sample = self._sample
        if sample is not None and len(sample) > 0:
            bucket = _upper_bound_sample(sample, key) - 1
            lo = max(0, bucket * self._stride)
            hi = min(lo + self._stride, count)
        else:
            lo, hi = 0, count
...
# ═══════════════════════════════════════════════════════════════════════════
# Local mapping lookups (no caching needed — bisect is sub-microsecond)
# ═══════════════════════════════════════════════════════════════════════════
