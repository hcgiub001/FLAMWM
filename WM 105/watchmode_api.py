...
class _ConnectionPool:
    ...
    def _get_once(
        ...
        # For HTTP through a proxy the request line must carry the
        # full URL so the proxy knows where to forward.  HTTPS
        # tunnels (via CONNECT) are transparent — use normal path.
...
class PackedMapping:
    ...
    def load_ram(self, spec: Dict[str, str]) -> None:
        """Load all files fully into RAM arrays, then build the
        direct-map."""
        self.close()
        self._is_mmap = False
    ...
    def _build_direct_map(self) -> None:
        ...
        # Backward fill: empty buckets inherit next bucket's start.
...
def ensure_loaded(mode: Optional[str] = None, force: bool = False) -> None:
    """Make sure all three mapping tables are loaded.  Thread-safe,
    idempotent."""
    global _runtime_mode, _dataset_id
...
# In-flight dedup: one lock per cache key prevents duplicate parallel
# API calls.
...
def _cached_api_get(
    ...
    # Acquire per-key lock so identical requests don't fire in
    # parallel.
    with _inflight_lock:
...
# delegates to get_watchmode_bundle_for_addon and then returns whichever
...
def _empty_bundle() -> Dict[str, Any]:
    """Return a bundle with no results and None for every extras
    field."""
    return {
        "results":          [],
        "total_results":    0,
...
def get_watchmode_bundle_for_addon(
    ...
    # ── Step 2: Call Watchmode API (only network wait) ─────────
    try:
