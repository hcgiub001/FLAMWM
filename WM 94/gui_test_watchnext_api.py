...
# -*- coding: utf-8 -*-
"""
gui_test_watchnext_api.py — Desktop tester for watchnext_api.py.

Place alongside watchnext_api.py.

Run:  python gui_test_watchnext_api.py
"""
...
# ---------------------------------------------------------------------
# Import watchnext_api with robust stubbing fallback
# ---------------------------------------------------------------------
def import_watchnext_api_with_stubs() -> Any:
    try:
        import watchnext_api  # type: ignore
        return watchnext_api
    except Exception as exc:
        if not os.path.exists(os.path.join(_THIS_DIR, "watchnext_api.py")):
            raise RuntimeError(
                "watchnext_api.py not found in current directory. "
                "Place your watchnext_api.py here before running this tester."
            ) from exc
        print("Initial import of watchnext_api failed; installing stubs for testing. Error:", exc)
    ...
    try:
        watchnext_api = importlib.import_module("watchnext_api")
        print("Imported watchnext_api with stubs (testing mode).")
        return watchnext_api
    except Exception as exc2:
        raise RuntimeError(f"Could not import watchnext_api even after installing stubs: {exc2}") from exc2


try:
    wm = import_watchnext_api_with_stubs()
except Exception as e:
    raise SystemExit(f"Failed to import watchnext_api.py for testing: {e}")
...
def compute_watchmode_cache_dir() -> str:
    try:
        base = wm.translate_path("special://profile/addon_data/plugin.video.fenlight/")  # type: ignore[attr-defined]
        return os.path.join(base, "cache", "watchmode")
    except Exception:
        return os.path.join(os.path.expanduser("~"), "addon_data", "plugin.video.fenlight", "cache", "watchmode")
...
class App:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("WatchNext API Tester")
        ...
        ttk.Label(api_frame, text="(key change invalidates API cache)", foreground="gray").pack(
            side="left", padx=8
        )
        ...
        ttk.Combobox(ctrl_frame, textvariable=self.type_var, values=("movie", "tv"), width=8, state="readonly").grid(
        ...
        self.mode_combo = ttk.Combobox(
            ctrl_frame, textvariable=self.mode_var, values=("auto", "RAM", "mmap"), width=10, state="readonly"
        )
        self.mode_combo.grid(row=0, column=7, sticky="w", padx=(4, 0))
        self.mode_combo.bind("<<ComboboxSelected>>", self.on_mode_selected)

        # ---- WatchNext output selector ----
        ttk.Label(ctrl_frame, text="WatchNext output:").grid(row=0, column=8, sticky="w", padx=(14, 0))
        self.watchnext_format_var = tk.StringVar(value="addon")
        self.watchnext_format_combo = ttk.Combobox(
            ctrl_frame,
            textvariable=self.watchnext_format_var,
            values=("addon", "pairs", "packed", "legacy_resolved"),
            width=16,
            state="readonly",
        )
        self.watchnext_format_combo.grid(row=0, column=9, sticky="w", padx=(4, 0))
        ...
        ttk.Checkbutton(
            cacheopt,
            text="Bypass API cache (details/search/similar; forces HTTP)",
        ...
        ttk.Label(cacheopt, textvariable=self.cache_version_var, foreground="gray").grid(
            row=0, column=2, sticky="e", padx=6
        )
        cacheopt.grid_columnconfigure(2, weight=1)

        # ---- TTL editor ----
        ttl_frame = ttk.LabelFrame(root, text="API cache TTLs (hours) \u2014 edits settings_cache keys")
        ttl_frame.pack(fill="x", padx=8, pady=4)

        self.ttl_search_var = tk.StringVar(value="")
        ...
        def ttl_entry(col: int, label: str, var: tk.StringVar):
            ttk.Label(ttl_frame, text=label).grid(row=0, column=col, sticky="w", padx=(6, 2), pady=2)
            ttk.Entry(ttl_frame, textvariable=var, width=8).grid(row=0, column=col + 1, sticky="w", padx=(0, 12))

        ttl_entry(0, "Search:", self.ttl_search_var)
        ttl_entry(2, "Details:", self.ttl_details_var)
        ttl_entry(4, "Similar(packed):", self.ttl_similar_var)
        ttl_entry(6, "Negative:", self.ttl_negative_var)

        self.save_ttls_btn = ttk.Button(ttl_frame, text="Save TTLs", command=self.on_save_ttls)
        self.save_ttls_btn.grid(row=0, column=8, sticky="w", padx=6)

        ttk.Label(ttl_frame, text="Tip: change TTLs then Clear Cache to fully apply.", foreground="gray").grid(
        ...
        self.api_search_btn = ttk.Button(btn_frame, text="TMDB \u2192 WM (API search)", command=self.on_api_search)
        self.api_search_btn.pack(side="left", padx=4)

        self.details_btn = ttk.Button(btn_frame, text="WM Details (API)", command=self.on_details)
        self.details_btn.pack(side="left", padx=4)

        self.watchnext_btn = ttk.Button(btn_frame, text="Run WatchNext", command=self.on_watchnext)
        self.watchnext_btn.pack(side="left", padx=4)
    ...
    def refresh_cache_labels(self):
        try:
            ...
            if hasattr(self.api, "main_cache"):
                v = self.api.main_cache.get("watchmode:cache_version")  # type: ignore[attr-defined]
            if v is None:
                v = "?"
    ...
    def refresh_ttl_fields_from_api(self):
        # Prefer dataset_info() (effective TTLs)
        try:
            info = self.api.dataset_info() if hasattr(self.api, "dataset_info") else {}
            ttls = info.get("cache_ttls_hours", {}) if isinstance(info, dict) else {}
            if isinstance(ttls, dict) and ttls:
                self.ttl_search_var.set(str(ttls.get("search", "")))
                self.ttl_details_var.set(str(ttls.get("details", "")))
                self.ttl_similar_var.set(str(ttls.get("similar_packed", "")))
                self.ttl_negative_var.set(str(ttls.get("negative", "")))
                return
        except Exception:
            pass
        ...
        if sc_get_setting is None:
            return
        try:
            self.ttl_search_var.set(sc_get_setting("fenlight.watchmode.cache.search_hours", "720"))
            self.ttl_details_var.set(sc_get_setting("fenlight.watchmode.cache.details_hours", "168"))
            self.ttl_similar_var.set(sc_get_setting("fenlight.watchmode.cache.similar_hours", "24"))
            self.ttl_negative_var.set(sc_get_setting("fenlight.watchmode.cache.negative_hours", "24"))
        except Exception:
            pass
    ...
    def on_save_key(self):
        ...
        try:
            if hasattr(self.api, "set_api_key"):
                ...
                if ok:
                    self.refresh_cache_labels()
                    messagebox.showinfo("Saved", "API key saved successfully (API cache invalidated).")
                else:
                    messagebox.showerror("Error", "Failed to save API key.")
            else:
                messagebox.showwarning("Warning", "watchnext_api has no set_api_key function.")
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def on_save_ttls(self):
        if sc_set_setting is None:
            messagebox.showerror("Unavailable", "settings_cache.set_setting is not available in this environment.")
            return
        ...
        try:
            sc_set_setting("fenlight.watchmode.cache.search_hours", norm_hours(self.ttl_search_var.get()))
            sc_set_setting("fenlight.watchmode.cache.details_hours", norm_hours(self.ttl_details_var.get()))
            sc_set_setting("fenlight.watchmode.cache.similar_hours", norm_hours(self.ttl_similar_var.get()))
            sc_set_setting("fenlight.watchmode.cache.negative_hours", norm_hours(self.ttl_negative_var.get()))
            messagebox.showinfo("Saved", "TTL settings saved. (Existing cached items keep old expiry until cleared.)")
            self.on_info()
        except Exception as e:
    ...
    def on_mode_selected(self, event=None):
        ...
        if persisted:
            if messagebox.askyesno("Mode changed", f"Mode set to '{new_mode}'. Reload mappings now to apply?"):
                self.on_reload()
        else:
            messagebox.showwarning("Persist failed", "Could not persist mode selection.")
    ...
    def on_clear(self):
        try:
            ...
            self.refresh_cache_labels()
            self.set_meta("Cache cleared (cache_version bumped + mapping files deleted).")
            self.set_result(json.dumps({"cache_cleared": True, "mapping_cache_dir_deleted": cache_dir}, indent=2))
        except Exception as e:
        ...
        def task():
            try:
                ...
                if result:
                    ...
                    self.set_meta(
                        f"WM {wm_id} \u2192 TMDB {result['tmdb_id']} ({result['media_type']}) ({elapsed_ms:.3f} ms)"
                    )
                else:
    ...
    def on_api_search(self):
        ...
        def task():
            try:
                self.set_status("calling TMDB \u2192 WM (API search, cached)...")
                self._set_buttons_state(False)
                ...
                t0 = time.perf_counter()
                wm_id = self.api.search_watchmode_via_api(
                    tmdb_id,
                    kind,
                    api_key=api_key,
                    bypass_cache=bypass,
                ...
                self.set_result(json.dumps(out, indent=2))
                self.set_meta(f"API search: TMDB {tmdb_id} \u2192 WM {wm_id or 'not found'} ({elapsed_ms:.1f} ms)")
                if wm_id is not None:
                    self.wm_id_var.set(str(wm_id))
    ...
    def on_details(self):
        wm_id = safe_int(self.wm_id_var.get())
        if wm_id is None:
            messagebox.showerror("Invalid", "Enter a Watchmode ID first (or use TMDB \u2192 WM).")
            return

        def task():
            try:
                self.set_status("calling WM details API (cached)...")
                self._set_buttons_state(False)
                ...
                bypass = bool(self.bypass_api_cache_var.get())
                t0 = time.perf_counter()
                details = self.api.call_details_api(wm_id, api_key=api_key, bypass_cache=bypass)
                elapsed_ms = (time.perf_counter() - t0) * 1000.0
                ...
                if details:
                    ...
                    similar_count = len(details.get("similar_titles", []) or [])
    ...
    # ---- WatchNext pipeline ----
    def on_watchnext(self):
        tmdb_id = safe_int(self.tmdb_id_var.get())
        if tmdb_id is None:
            messagebox.showerror("Invalid", "Enter a numeric TMDB ID.")
            return
        kind = self.type_var.get()
        fmt = (self.watchnext_format_var.get() or "addon").strip()

        def task():
            try:
                self.set_status("running WatchNext pipeline...")
                self._set_buttons_state(False)
                self.api.ensure_loaded(mode=self.mode_var.get())
                ...
                if fmt == "addon":
                    if not hasattr(self.api, "get_watchnext_for_addon"):
                        raise RuntimeError("watchnext_api.get_watchnext_for_addon not found (API mismatch)")
                    result = self.api.get_watchnext_for_addon(
                        tmdb_id, kind, api_key=api_key, bypass_cache=bypass
                    )

                elif fmt == "pairs":
                    if not hasattr(self.api, "query_watchnext_pairs"):
                        raise RuntimeError("watchnext_api.query_watchnext_pairs not found (API mismatch)")
                    result = self.api.query_watchnext_pairs(
                        tmdb_id, kind, api_key=api_key, bypass_cache=bypass
                    )

                elif fmt == "packed":
                    if not hasattr(self.api, "query_watchnext_packed"):
                        raise RuntimeError("watchnext_api.query_watchnext_packed not found (API mismatch)")
                    packed = self.api.query_watchnext_packed(
                        tmdb_id, kind, api_key=api_key, bypass_cache=bypass
                    )
                    decoded = [{"id": (p >> 1), "media_type": ("tv" if (p & 1) else "movie")} for p in packed]
                    result = {"count": len(packed), "packed": packed, "decoded": decoded}

                else:  # legacy_resolved
                    if not hasattr(self.api, "get_similar_titles_resolved"):
                        raise RuntimeError("watchnext_api.get_similar_titles_resolved not found (API mismatch)")
                    result = self.api.get_similar_titles_resolved(
                        tmdb_id, kind, api_key=api_key, bypass_cache=bypass
                    )
                ...
                count = None
                if isinstance(result, dict):
                    count = result.get("total_results") or result.get("count") or result.get("similar_count")
                self.set_meta(
                    f"WatchNext: TMDB {tmdb_id} ({kind}) \u2014 format={fmt} \u2014 count={count} ({elapsed_ms:.1f} ms)"
                )
