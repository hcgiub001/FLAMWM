...
# drops idle connections after _CONN_IDLE_TIMEOUT_S)
# ═══════════════════════════════════════════════════════════════════════════
class _ConnectionPool:
    """Thread-safe HTTP/HTTPS connection pool with keep-alive, redirect
    support, and automatic eviction of connections idle longer than
    _CONN_IDLE_TIMEOUT_S seconds."""
...
def _search_tmdb_on_watchmode(tmdb_id: int, kind: str, api_key: str,
    ...
    """Search Watchmode API for a TMDB ID, return the Watchmode title ID.

    Note: The main query flow no longer calls this function — it uses
    the TMDB-format ID (e.g. ``movie-957``) directly in the details
    endpoint instead.  This helper is retained for diagnostic or
    special-case usage.
    """
    ...
    data = _call_api("/search/", params, api_key,
                     namespace="search", ttl_hours=_search_cache_hours(),
                     bypass=bypass_cache)
    if not data:
        return None
