...
def import_watchnext_api_with_stubs() -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not import watchnext_api even after installing stubs: {exc2}"
        ) from exc2
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        ttk.Label(ctrl_frame, text="WatchNext output:").grid(
            row=0, column=8, sticky="w", padx=(14, 0)
        )
        self.watchnext_format_var = tk.StringVar(value="bundle")
        ...
        self.watchnext_btn = ttk.Button(
            btn_frame, text="Run WatchNext", command=self.on_watchnext
        )
        ...
        try:
            if hasattr(self.api, "set_api_key"):
                ...
                messagebox.showwarning(
                    "Warning",
                    "watchnext_api has no set_api_key function.",
                )
        except Exception as e:
    ...
    def on_watchnext(self):
        ...
        if tmdb_id is None:
            ...
            return
        kind = self.type_var.get()
        fmt = (self.watchnext_format_var.get() or "bundle").strip()
        ...
        def task():
            try:
                ...
                if fmt == "bundle":
                    if not hasattr(self.api, "get_watchnext_bundle_for_addon"):
                        raise RuntimeError(
                            "watchnext_api.get_watchnext_bundle_for_addon "
                            "not found (API mismatch)"
                        )
                    result = self.api.get_watchnext_bundle_for_addon(
                        tmdb_id, kind,
                    ...
                    if not hasattr(self.api, "get_watchnext_for_addon"):
                        raise RuntimeError(
                            "watchnext_api.get_watchnext_for_addon "
                            "not found (API mismatch)"
                        )
                    ...
                    if not hasattr(self.api, "get_watchmode_extras"):
                        raise RuntimeError(
                            "watchnext_api.get_watchmode_extras "
                            "not found (API mismatch)"
                        )
                    ...
                    if not hasattr(self.api, "query_watchnext_pairs"):
                        raise RuntimeError(
                            "watchnext_api.query_watchnext_pairs "
                            "not found (API mismatch)"
                        )
                    ...
                    if not hasattr(self.api, "query_watchnext_packed"):
                        raise RuntimeError(
                            "watchnext_api.query_watchnext_packed "
                            "not found (API mismatch)"
                        )
